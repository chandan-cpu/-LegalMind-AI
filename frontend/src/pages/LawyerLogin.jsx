...
export default function LawyerLoginPage() {
  ...
  const handleSendResetOtp = async (e) => {
    ...
    try {
      const res = await lawyerAPI.sendResetOTP(forgotEmail);
  ...
  const handleVerifyResetOtp = async (e) => {
    ...
    setForgotLoading(true);
    try {
      const res = await lawyerAPI.verifyResetOTP(forgotOtp, sessionToken);
      setSessionToken(res.data.sessionToken);
  ...
  const handleResetPassword = async (e) => {
    ...
    setForgotLoading(true);
    try {
      await lawyerAPI.resetPassword(newPassword, sessionToken);
  ...
  const stepSubtitles = [
    'Enter your registered lawyer email to receive an OTP.',
              ...
              <form onSubmit={handleSendResetOtp} className="auth-form" style={{ marginTop: '1.5rem' }}>
                <div className="input-group">
                  ...
                  <input
                    type="email"
                    className="input-field input-with-icon"
                    placeholder="Registered lawyer email"
                    value={forgotEmail}
